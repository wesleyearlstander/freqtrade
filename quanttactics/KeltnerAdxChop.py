...
# ==========================================
# Keltner Channel Strategy: 126% Profit in Backtest! (Full Tutorial)
# https://youtu.be/19mZnAj23po
# ==========================================
...
freqtrade download-data \
    -c user_data/binance_futures_KeltnerAdxChop.json \
    --timerange 20230101- \
...
"""
freqtrade hyperopt \
    --strategy KeltnerAdxChop \
    --config user_data/binance_futures_KeltnerAdxChop.json \
    --timeframe 1h \
...
class KeltnerAdxChop(IStrategy):
    ...
    kc_period = CategoricalParameter([15, 20, 25, 30], default=20, space="buy")
    kc_atr_mult = CategoricalParameter([1.5, 2, 2.5], default=2, space="buy")  
    ...
    def plot_config(self):
        return {
            "main_plot": {
                f'kc_upperband_{self.kc_period.value}_{self.kc_atr_mult.value}': {"color": "#2962FF"},
                f'kc_lowerband_{self.kc_period.value}_{self.kc_atr_mult.value}': {"color": "#2962FF"}
            },
            "subplots": {
    ...
    def populate_indicators(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        for kc_period_val in self.kc_period.range: 
            for kc_atr_mult_val in self.kc_atr_mult.range: 

                keltner = qtpylib.keltner_channel(
                    dataframe,
                    window=kc_period_val,
                    atrs=kc_atr_mult_val
                )
                dataframe[f"kc_upperband_{kc_period_val}_{kc_atr_mult_val}"] = keltner["upper"]
                dataframe[f"kc_lowerband_{kc_period_val}_{kc_atr_mult_val}"] = keltner["lower"]
    ...
    def populate_entry_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:        
        ...
        dataframe.loc[
                (
                    (qtpylib.crossed_above(dataframe["close"], dataframe[f"kc_upperband_{self.kc_period.value}_{self.kc_atr_mult.value}"])) &
        ...
        dataframe.loc[
                (
                    (qtpylib.crossed_below(dataframe["close"], dataframe[f"kc_lowerband_{self.kc_period.value}_{self.kc_atr_mult.value}"])) &
                    (dataframe['adx'] > self.adx_threshold.value) &
                    (dataframe['chop'] < self.chop_threshold.value) &
                    (dataframe['volume'] > 0)
                ),
