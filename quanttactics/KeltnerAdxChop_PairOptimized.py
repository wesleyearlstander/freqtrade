...
freqtrade download-data \
    -c user_data/binance_futures_KeltnerAdxChop_PairOptimized.json \
    --timerange 20230101- \
...
freqtrade backtesting \
    --strategy KeltnerAdxChop_PairOptimized \
    --timeframe 1h \
...
"""
freqtrade webserver \
    --config user_data/binance_futures_KeltnerAdxChop_PairOptimized.json
"""


class KeltnerAdxChop_PairOptimized(IStrategy):
    ...
    def plot_config(self):
        return {
            "main_plot": {
                'kc_upperband': {"color": "#2962FF"},
                'kc_lowerband': {"color": "#2962FF"}
            },
            "subplots": {
                "ADX": {
    ...
    def populate_indicators(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        if pair in self.custom_info:
            ...
            keltner = qtpylib.keltner_channel(
                dataframe,
                window=pair_settings["kc_period"],
                atrs=pair_settings["kc_atr_mult"]
            )
    ...
    def populate_entry_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:        
        ...
        if pair in self.custom_info:
            ...
            dataframe.loc[
                    (
                        (qtpylib.crossed_below(dataframe["close"], dataframe["kc_lowerband"])) &
                        (dataframe['adx'] > pair_settings["adx_threshold"]) &
                        (dataframe['chop'] < pair_settings["chop_threshold"]) &
                        (dataframe['volume'] > 0)
                    ),
