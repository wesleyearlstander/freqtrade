...
freqtrade download-data \
    -c user_data/binance_futures_EMA8_13_21_MACD_Strategy_PairOptimized.json \
    --timerange 20230101- \
...
freqtrade backtesting \
    --strategy EMA8_13_21_MACD_Strategy_PairOptimized \
    --timeframe 1h \
...
"""
freqtrade webserver \
    --config user_data/binance_futures_EMA8_13_21_MACD_Strategy_PairOptimized.json
"""

class EMA8_13_21_MACD_Strategy_PairOptimized(IStrategy):
        ...
        if pair in self.custom_info:
            ...
            dataframe.loc[
                (
                    ...
                    (dataframe["ema8"].rolling(window=pair_settings["ema_rolling_window"]).apply(
                    ...
                    (dataframe["ema13"].rolling(window=pair_settings["ema_rolling_window"]).apply(
                        lambda x: any(qtpylib.crossed_above(x, dataframe["ema21"].iloc[x.index[0]:x.index[-1]+1])))) &
        ...
        if pair in self.custom_info:
            ...
            if take_profit is None or stop_loss is None:
                ...
                if not trade.is_short:
                    ...
                    swing_low = signal_data["low"].rolling(pair_settings["swing_point_lookback"]).min().iloc[-1]
                    ...
                    swing_high = signal_data["high"].rolling(pair_settings["swing_point_lookback"]).max().iloc[-1]
