...
freqtrade download-data \
    -c user_data/binance_futures_ADX_OBV_PairOptimized.json \
    --timerange 20230101- \
...
class ADX_OBV_PairOptimized(IStrategy):
    ...
    def populate_indicators(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        if pair in self.custom_info:
            ...
            dataframe["obv_ma"] = dataframe["obv"].rolling(window=pair_settings['obv_ma_period']).mean()
    ...
    def populate_entry_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:        
        ...
        if pair in self.custom_info:
            ...
            dataframe.loc[
                (   
                    (dataframe[f"adx"] > pair_settings["adx_threshold"]) &
                    (dataframe['plus_di'] > dataframe["minus_di"]) &
                    (dataframe['plus_di'].rolling(window=pair_settings["di_cross_window"]).apply(
