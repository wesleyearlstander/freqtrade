...
# ==========================================
# ADX + OBV Strategy Generated 336% Profit (Full Tutorial) 
# https://youtu.be/6B-T2BVb5VA
# ==========================================
...
freqtrade download-data \
    -c user_data/binance_futures_ADX_OBV.json \
    --timerange 20230101- \
...
class ADX_OBV(IStrategy):
    ...
    adx_threshold = CategoricalParameter([25, 30, 35], default=25, space="buy")
    di_cross_window = CategoricalParameter([5, 10, 15, 20, 25], default=15, space="buy")
    
    obv_ma_period = CategoricalParameter([80, 100, 120], default=100, space="buy")
        ...
        plot_config = {
            ...
            "subplots": {
                ...
                "OBV": {
                    ...
                    f"obv_ma{self.obv_ma_period.value}": {
                        "color": "#fdd835",
                        "type": "line"
                    }
                }
    ...
    def populate_indicators(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        for val in self.obv_ma_period.range:
            dataframe[f"obv_ma{val}"] = dataframe["obv"].rolling(window=val).mean()
    ...
    def populate_entry_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:        
        ...
        dataframe.loc[
            (   
                ...
                (dataframe['plus_di'].rolling(window=self.di_cross_window.value).apply(
                    lambda x: any(qtpylib.crossed_above(x, dataframe['minus_di'].iloc[x.index[0]:x.index[-1]+1])) 
                )) & 
                (dataframe["obv"] > dataframe[f"obv_ma{self.obv_ma_period.value}"]) &
                (dataframe["volume"] > 0)
        ...
        dataframe.loc[
            (
                ...
                (dataframe['plus_di'] < dataframe["minus_di"]) &
                (dataframe['plus_di'].rolling(window=self.di_cross_window.value).apply(
                    lambda x: any(qtpylib.crossed_below(x, dataframe['minus_di'].iloc[x.index[0]:x.index[-1]+1])) 
                )) &
                (dataframe["obv"] < dataframe[f"obv_ma{self.obv_ma_period.value}"]) &
                (dataframe["volume"] > 0)
            ),
