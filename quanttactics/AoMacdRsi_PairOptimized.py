...
freqtrade download-data \
    -c user_data/binance_futures_AoMacdRsi_PairOptimized.json \
    --timerange 20230101- \
...
freqtrade backtesting \
    --strategy AoMacdRsi_PairOptimized \
    --timeframe 1h \
...
"""
freqtrade webserver \
    --config user_data/binance_futures_AoMacdRsi_PairOptimized.json
"""


class AoMacdRsi_PairOptimized(IStrategy):
    ...
    def populate_entry_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        if pair in self.custom_info:
            ...
            dataframe.loc[
                    (
                        (dataframe['ao'] > 0) &
                        (dataframe["ao"].rolling(window=pair_settings["ao_rolling_window"]).apply(
                        lambda x: any(qtpylib.crossed_above(x, 0)))) &
            
                        (dataframe['rsi'] > pair_settings["rsi_threshold"]) &
                        (dataframe["rsi"].rolling(window=pair_settings["rsi_macd_rolling_window"]).apply(
                            lambda x: any(qtpylib.crossed_above(x, pair_settings["rsi_threshold"])) 
                        )) &

                        (dataframe["macd"] > dataframe["macdsignal"]) &
                        (dataframe["macd"].rolling(window=pair_settings["rsi_macd_rolling_window"]).apply(
            ...
            dataframe.loc[
                    (
                        ...
                        (dataframe["rsi"].rolling(window=pair_settings["rsi_macd_rolling_window"]).apply(
                            lambda x: any(qtpylib.crossed_below(x, (100 - pair_settings["rsi_threshold"]))) 
                        )) &
