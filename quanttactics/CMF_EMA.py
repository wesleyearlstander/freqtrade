...
freqtrade download-data \
    -c user_data/binance_futures_CMF_EMA.json \
    --timerange 20230101- \
...
freqtrade hyperopt \
    --strategy CMF_EMA \
    --config user_data/binance_futures_CMF_EMA.json \
    --timeframe 1h \
...
class CMF_EMA(IStrategy):
    ...
    cmf_long_threshold = CategoricalParameter([0.03, 0.05, 0.07, 0.09], default=0.05, space="buy", optimize=True) 
    cmf_short_threshold = CategoricalParameter([-0.03, -0.05, -0.07, -0.09], default=-0.05, space="buy", optimize=True) 
    ...
    def plot_config(self):
        return {
            ...
            "subplots": {
                "CMF": {
                    f"cmf{self.cmf_length.value}": {"color": "#7e57c2"}
                }
            }
        }
    ...
    def populate_indicators(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        for val in self.cmf_length.range:
            dataframe[f"cmf{val}"] = pta.cmf(high=dataframe['high'], low=dataframe['low'], close=dataframe['close'], volume=dataframe['volume'], length=val)
    ...
    def populate_entry_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        dataframe.loc[
            (
                (qtpylib.crossed_above(dataframe[f"cmf{self.cmf_length.value}"], self.cmf_long_threshold.value)) &
        ...
        dataframe.loc[
            (
                (qtpylib.crossed_below(dataframe[f"cmf{self.cmf_length.value}"], self.cmf_short_threshold.value)) &
                (dataframe["close"] < dataframe["ema200"]) &
                (dataframe["volume"] > 0) 
            ),
            "enter_short"] = 1
