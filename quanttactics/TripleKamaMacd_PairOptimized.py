...
freqtrade download-data \
    -c user_data/binance_futures_TripleKamaMacd_PairOptimized.json \
    --timerange 20230101- \
...
freqtrade backtesting \
    --strategy TripleKamaMacd_PairOptimized \
    --timeframe 1h \
...
"""
freqtrade webserver \
    --config user_data/binance_futures_TripleKamaMacd_PairOptimized.json
"""


class TripleKamaMacd_PairOptimized(IStrategy):
        ...
        if pair in self.custom_info:
            ...
            dataframe.loc[
                (
                    ...
                    (dataframe["kama8"].rolling(window=pair_settings["kama_rolling_window"]).apply(
                    ...
                    (dataframe["kama13"].rolling(window=pair_settings["kama_rolling_window"]).apply(
                        lambda x: any(qtpylib.crossed_above(x, dataframe["kama21"].iloc[x.index[0]:x.index[-1]+1])))) &
