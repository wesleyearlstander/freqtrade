...
# ==========================================
# Aroon Indicator Trading Strategy 291% Profit! (Full Guide)
# https://youtu.be/9XjUkZ3imME
# ==========================================
...
freqtrade download-data \
    -c user_data/binance_futures_AroonMacd_PairOptimized.json \
    --timerange 20230101- \
...
freqtrade backtesting \
    --strategy AroonMacd_PairOptimized \
    --timeframe 1h \
...
"""
freqtrade webserver \
    --config user_data/binance_futures_AroonMacd_PairOptimized.json
"""


class AroonMacd_PairOptimized(IStrategy):
    ...
    def plot_config(self):
        ...
        plot_config = {
            ...
            "subplots": {
                ...
                "Aroon": {
                    "aroonup": {"color": "#FB8C00", "type": "line"},
                    "aroondown": {"color": "#2962FF", "type": "line"}
                },
                "Oscillator": {
                    "aroonosc": {"color": "red", "type": "line"}
                }
            }
        }
        ...
        if pair in self.custom_info:
            ...
            dataframe.loc[
                (   
                    ...
                    (dataframe["aroonup"].rolling(window=pair_settings["aroon_cross_window"]).apply(
                        lambda x: any(qtpylib.crossed_above(x, dataframe["aroondown"].iloc[x.index[0]:x.index[-1]+1])) 
                    )) & 
                    
                    (dataframe["aroonosc"] > 0) &
                    (dataframe["aroonosc"] > dataframe["aroonosc"].shift(1)) &
                    
                    (dataframe["macd"] > dataframe["macdsignal"]) &
                    
                    (dataframe["macd"].rolling(window=pair_settings["macd_cross_window"]).apply(
            ...
            dataframe.loc[
                (   
                    ...
                    (dataframe["aroonup"].rolling(window=pair_settings["aroon_cross_window"]).apply(
                        lambda x: any(qtpylib.crossed_below(x, dataframe["aroondown"].iloc[x.index[0]:x.index[-1]+1])) 
                    )) & 
