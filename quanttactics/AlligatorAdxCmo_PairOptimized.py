...
freqtrade download-data \
    -c user_data/binance_futures_AlligatorAdxCmo_PairOptimized.json \
    --timerange 20230101- \
...
freqtrade backtesting \
    --strategy AlligatorAdxCmo_PairOptimized \
    --timeframe 1h \
...
"""
freqtrade webserver \
    --config user_data/binance_futures_AlligatorAdxCmo_PairOptimized.json
"""

class AlligatorAdxCmo_PairOptimized(IStrategy):
    ...
    def populate_indicators(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        if pair in self.custom_info:
            ...
            dataframe[f"ema"] = ta.EMA(dataframe, timeperiod=pair_settings["ema_period"])
        ...
        if pair in self.custom_info:
            ...
            dataframe.loc[
                    (
                        ...
                        (bullish_mouth_closed.shift(1).rolling(pair_settings["mouth_closed_lookback"]).max() == True) &
                        
                        (dataframe['close'] > dataframe["ema"]) &
                        (dataframe['adx'] > pair_settings["adx_threshold"]) &
                        (dataframe["cmo"] > pair_settings["cmo_threshold"]) &
                        (dataframe['volume'] > 0)
                    ),
                    'enter_long'] = 1
            ...
            dataframe.loc[
                    (
                        ...
                        (dataframe['lips'] < dataframe['teeth']) &
                        (dataframe['teeth'] < dataframe['jaw']) &
                        (bearish_mouth_closed.shift(1).rolling(pair_settings["mouth_closed_lookback"]).max() == True) &
